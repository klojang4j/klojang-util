...
/**
 * The {@code list-of-int} interface. {@code IntList} is a sealed interface with just
 * two implementations: one ({@link IntArrayList}) allows mutations on the list and
 * the other is immutable. Instances of the immutable variant can only be obtained
 * through static factory methods on the {@code IntList} interface.
 */
public sealed interface IntList extends Emptyable permits IntArrayList,
   ...
   * current capacity, and even less than the current size of the list. So this
   * method can be used as a truncate or a trim-to-size method.
   *
